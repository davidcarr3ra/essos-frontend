			...
			{Array.from(new Set(flightOptions.map(f => f.id))).map((flightId) => {
				...
				return (
					<TableRow key={flightId}>
						...
						<TableCell>
							<RadioGroup value={selectedFlight?.id === flightId && selectedFlight.class === 'economy' ? 'selected' : ''}>
								<div className="flex items-center space-x-2">
									<RadioGroupItem 
										value="selected" 
										id={`economy-${flightId}`} 
										onClick={() => setSelectedFlight(flightGroup.find(f => f.class === 'economy') || null)} 
									/>
									<Label htmlFor={`economy-${flightId}`}>${flightGroup.find(f => f.class === 'economy')?.price}</Label>
								</div>
							</RadioGroup>
						</TableCell>
						<TableCell>
							<RadioGroup value={selectedFlight?.id === flightId && selectedFlight.class === 'business' ? 'selected' : ''}>
								<div className="flex items-center space-x-2">
									<RadioGroupItem 
										value="selected" 
										id={`business-${flightId}`} 
										onClick={() => setSelectedFlight(flightGroup.find(f => f.class === 'business') || null)} 
									/>
									<Label htmlFor={`business-${flightId}`}>${flightGroup.find(f => f.class === 'business')?.price}</Label>
								</div>
							</RadioGroup>
						</TableCell>
						<TableCell>
							<RadioGroup value={selectedFlight?.id === flightId && selectedFlight.class === 'firstClass' ? 'selected' : ''}>
								<div className="flex items-center space-x-2">
									<RadioGroupItem 
										value="selected" 
										id={`firstClass-${flightId}`} 
										onClick={() => setSelectedFlight(flightGroup.find(f => f.class === 'firstClass') || null)} 
									/>
									<Label htmlFor={`firstClass-${flightId}`}>${flightGroup.find(f => f.class === 'firstClass')?.price}</Label>
								</div>
							</RadioGroup>
						</TableCell>
					</TableRow>
...
export function FlightsSection({
  ...
  return (
    <div className="space-y-6">
      ...
      {!skipFlights && (
        <>
          <Tabs defaultValue="roundTrip" onValueChange={(value) => setFlightType(value as 'oneWay' | 'roundTrip')}>
            <TabsList>
