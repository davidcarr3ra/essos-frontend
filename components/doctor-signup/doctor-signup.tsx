...
import { Stethoscope, Globe, DollarSign, ShieldCheck, ArrowRight, ArrowLeft, Check, Upload } from 'lucide-react'
...
const formSchema = z.object({
  ...
  medicalAssociations: z.string().min(2, { message: "Please list at least one medical association." }),
  photoId: z
    .instanceof(FileList)
      ...
      <div className="grid md:grid-cols-2 gap-8">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Expand your practice globally with our medical tourism platform</CardDescription>
          </CardHeader>
          ...
          <CardContent>
            {!isSubmitted ? (
                ...
                {step === 3 && (
                  <>
                    ...
                    <div>
                      ...
                      <Textarea id="medicalAssociations" {...register("medicalAssociations")} placeholder="List the medical associations you are a member of, separated by commas" />
                      {errors.medicalAssociations && <p className="text-sm text-red-500">{errors.medicalAssociations.message}</p>}
                    </div>
                  </>
                )}
                {step === 4 && (
                  <>
                    <div>
                      ...
                      <Input id="photoId" type="file" {...register("photoId")} accept=".pdf,.jpg,.jpeg,.png" />
                      {errors.photoId && <p className="text-sm text-red-500">{errors.photoId.message?.toString()}</p>}
                    </div>
                    <div>
                      <Label htmlFor="medicalCertificates">Medical Certificates</Label>
                      <Input id="medicalCertificates" type="file" {...register("medicalCertificates")} accept=".pdf,.jpg,.jpeg,.png" />
                      {errors.medicalCertificates && <p className="text-sm text-red-500">{errors.medicalCertificates.message?.toString()}</p>}
                    </div>
                    <div>
                      <Label htmlFor="cv">CV (PDF only)</Label>
                      <Input id="cv" type="file" {...register("cv")} accept=".pdf" />
                      {errors.cv && <p className="text-sm text-red-500">{errors.cv.message?.toString()}</p>}
                    </div>
                  </>
                )}
              </form>
              ...
              <div className="text-center space-y-4">
                ...
                <p>Thank you for your interest in joining our platform. Our team will review your application and get back to you within 3-5 business days.</p>
              </div>
            )}
          </CardContent>
